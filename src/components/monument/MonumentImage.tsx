...
const MonumentImage: React.FC<MonumentImageProps> = ({ image, name, location }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto opacity-0 animate-fade-in" style={{ animationDelay: '400ms', opacity: '1' }}>
      {image ? (
        imageError ? (
          <div className="relative w-full h-96 bg-gray-100 flex items-center justify-center rounded-md shadow-lg">
            <p className="text-gray-500">Image not available</p>
          </div>
        ) : (
          <>
            {!imageLoaded && <Skeleton className="w-full h-96 rounded-md" />}
            <img 
              src={image}
